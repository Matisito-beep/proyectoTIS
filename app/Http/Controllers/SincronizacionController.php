...
class SincronizacionController extends Command
{
    /**
    ...
    private function sincronizarProfesores()
    {
        ...
        while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
            ...
            if (!$profesorExistente) {
                ProfesorDinf::create([
                    'rut_profesor' => $row['RUT_PROFESOR_DINF'],
                    'nombre_profesor' => $row['NOMBRE_PROFESOR_DINF'],
                    'password' => substr($row['RUT_PROFESOR_DINF'], 0, -1),
                ]);
                $nuevos++;
            }
        }
